...
export const getEnvironment = async (files, html = aboutBlank) => {
    ...
    for(const file of files) {
        //TODO instrumenting
        dom.window.eval(await instrument(new URL(file, import.meta.url), 'utf8'));
    }
    return dom;
};
...
export const cleanUp = async (window) => {
    if(process.env.NYC_CONFIG) {
        ...
        await fs.writeFile(path.join(nycConfig.tempDir, `${Date.now()}_${process.pid}_${++id}.json`), JSON.stringify(window.__coverage__), 'utf8');
    }
    window.close();
};
